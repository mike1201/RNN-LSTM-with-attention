...
#linear = tf.contrib.rnn.python.ops.core_rnn_cell_impl # Basic RNN-linear 
...
# 1. Define "attention_RNN Layer."
def attention_RNN(encoder_outputs, # Get Encoder`s output and state
                  encoder_state, 
                  num_decoder_symbols,
                  sequence_length,
                  num_heads=1,
  ...
  if use_attention:
    ...
    if num_heads < 1: # What`s the number of heads?
      raise ValueError("With less than 1 heads, use a non-attention decoder.")
  
    # 1-1. Reshape "Encoder outputs" to put in attention. (Use 1-by-1 convolution)
    with tf.variable_scope(scope or "attention_RNN"):
      
    # 1-1-1. Encoder_outputs --) top_states --) attention_states
    ...
    # 1-1-2. attention_states --) hidden
      batch_size = tf.shape(top_states[0])[0] # Needed for reshaping.
      attn_length = attention_states.get_shape()[1].value # 2
    ...
    # 1-2. Using CNN, get hidden features. (decoder)
    ...
    # 1-3. Get attention weights and attention vectors
      def attention(query):
        ...
        for i in range(num_heads): # xrange --) range
          with tf.variable_scope("Attention_%d" % i):
            ...
            # 1-3-1. Define Y
            y = linear(query, attention_vec_size, True) # attention_vec_size = seq_length
            y = tf.reshape(y, [-1, 1, 1, attention_vec_size])
            
            # 1-3-2. Define attention weights (=a)
            e = tf.reduce_sum(
                v[i] * tf.tanh(hidden_features[i] + y), [2, 3])
            a = tf.nn.softmax(e)
            attn_weights.append(a)
            
            # 1-3-3. Calculate the attention-weighted vector d.
            d = tf.reduce_sum(
      ...
      # Check?
      batch_attn_size = tf.stack([batch_size, attn_size]) # [[batch_size], [attn_size]]
      attns = [tf.zeros(batch_attn_size, dtype=dtype)
               for _ in range(num_heads)] # xrange --) range
      for a in attns: # Ensure the second shape of attention vectors is set         
        a.set_shape([None, attn_size]) # set_shape == reshape
      ...
      # 3. Get the at
      for i in range(len(encoder_outputs)): # xrange --) range
        
        # 3-1. Reuse the variables defined in another loop.
        if i > 0:
          tf.get_variable_scope().reuse_variables() # Use Reuse variables
        
        # 3-2. Get weights and weighted vectors.
        if i == 0:
        ...
        # 3-3. Get the softmax result from RNN.
        output = tf.concat([ds[0], encoder_outputs[i]], 1) # ds[0] = ds
                                                           # why encoder_output is input
        # NOTE: here we temporarily assume num_head = 1
        with tf.variable_scope("AttnRnnOutputProjection"):
          logit = linear(output, num_decoder_symbols, True) # num_decoder_symbols = vocab_size
                                                            # Maybe probability.
...
# 4. Get decoder output(=logit) and loss value
